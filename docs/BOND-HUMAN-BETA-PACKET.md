...
- [ ] Founder uses `@testingCQ_bot` as the Project Q beta bot and and supplies its token through protected environment configuration, never in the testing topic or Git. Operator checks `getMe`, `/startq`, Mini App launch and webhook identity against that bot.
