...
Project Q continues to own campaign rules, scoring, allocations, burns, settlement and distribution. Oracle remains the authority for the cross-ecosystem identity facts. A verified Oracle X event is mirrored into `x_identity`, and Project Q's signed-message wallet proof is mirrored into the canonical payout row in `wallet_connections`. Conflicting, revoked or already-owned identities fail closed instead of being reassigned.
