...
test('missing, malformed, late and wrong-protocol commitments fail closed', () => {
  ...
  for (const rows of variants) {
    assert.equal(evaluateBondDrawCommitments(cycles, rows).ready, false);
  }
});

test('duplicate commitment cycles do not count as complete evidence', () => {
  const rows = [...commitments.slice(0, 4), { ...commitments[4], cycle_id: 4 }];
  const state = evaluateBondDrawCommitments(cycles, rows);
  assert.equal(state.ready, false);
  assert.match(state.blockers.join(' '), /duplicate commitment cycle|cycle 5 draw commitment missing/);
});
