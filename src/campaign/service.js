...
const PUBLIC_READINESS_KEYS = new Set([
  'rules', 'funding', 'registry', 'sources', 'dates', 'draw-commitments',
  'app', 'wallet', 'settlement', 'burn-rules', 'burn-progress', 'burn-verification',
]);
...
export async function getCampaignReadiness(client, env = process.env, { now = new Date() } = {}) {
  ...
  const [
    ...
    client.select(
      ...
      `?campaign_id=eq.${encodeURIComponent(id)}` +
        '&select=campaign_id,cycle_id,protocol_version,commit_hash,committed_at&order=cycle_id.asc&limit=5'
    ),
    client.select(
