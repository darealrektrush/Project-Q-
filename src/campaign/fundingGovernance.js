...
export async function getFundingGovernanceState(client, campaignId = CAMPAIGN_ID, {
  ...
  const [campaignRows, founderRows, proposalRows, finalizationRows] = await Promise.all([
    ...
    client.select('campaign_funding_proposals',
      `?campaign_id=eq.${encodeURIComponent(campaignId)}&select=id,vault_address,vault_base_units,squads_approval_threshold,squads_member_count,top_contributor_prize_lamports,conservation_vault_address,conservation_contribution_lamports,total_sol_commitment_lamports,conservation_attribution,evidence_hash,verified_at,created_at&order=created_at.desc,id.desc&limit=20`),
  ...
  return {
    ...
    proposal: proposal ? {
      ...
      conservationVaultAddress: proposal.conservation_vault_address,
      conservationContributionLamports: String(proposal.conservation_contribution_lamports),
      totalSolCommitmentLamports: String(proposal.total_sol_commitment_lamports),
      conservationAttribution: proposal.conservation_attribution,
...
export function buildFundingGovernanceText(state) {
  ...
  return [
    ...
    ...(proposal ? [
      ...
      `Conservation vault: ${shortAddress(proposal.conservationVaultAddress)}`,
      `SOL commitment: ${proposal.totalSolCommitmentLamports} lamports (1.10 SOL total)`,
      `Attribution: ${proposal.conservationAttribution}`,
