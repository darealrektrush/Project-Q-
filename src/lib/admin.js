  ...
  if (action === 'preflight') {
    if (callbackQuery.message.chat.type !== 'private') {
      return telegram.sendMessage(
        chatId,
        'Production preflight is available only to an authorized founder in a private Project Q chat.',
        { threadId }
      );
    }
    try {
      ...
      return telegram.editMessageText(chatId, messageId, buildProductionPreflightText(preflight), {
        replyMarkup: {
          inline_keyboard: [
            [{ text: '🔄 Refresh', callback_data: 'admin:preflight' }],
            [{ text: '⬅️ Back to Bond the Duck', callback_data: 'admin:campaign:bond' }],
          ],
        },
      });
    } catch (err) {
      console.error('production preflight unavailable', err.message);
      return telegram.sendMessage(
        chatId,
